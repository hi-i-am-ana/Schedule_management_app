...
usersRouter.get('/:id(\\d+)', (req, res) => {
  db.each('SELECT users.user_id, firstname, lastname, email, schedule_id, day, start_time, end_time FROM users LEFT JOIN schedules ON schedules.user_id = users.user_id WHERE users.user_id = $1 ORDER BY user_id, day ASC, start_time ASC, end_time ASC;', req.params.id, row => {
    const days = {
      1: 'Monday',
      2: 'Tuesday',
      3: 'Wednesday',
