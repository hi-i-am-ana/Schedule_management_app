...
app.get('/', (req, res) => {
  ...
  database.each('SELECT users.user_id, firstname, lastname, schedule_id, day, start_time, end_time FROM users LEFT JOIN schedules ON schedules.user_id = users.user_id ORDER BY user_id, day ASC, start_time ASC, end_time ASC;', [], row => {
    const days = {
      1: 'Monday',
      2: 'Tuesday',
      3: 'Wednesday',
